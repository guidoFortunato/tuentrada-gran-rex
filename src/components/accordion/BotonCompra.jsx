...
export const BotonCompra = ({ performances, styles }) => {
  ...
  const buyButtons = performances?.buyButtons;
  ...
  return (
    <a href={performances?.buyButtons[0]?.link} target="_blank">
      <button
        ...
        className={
          "text-white bg-gray-300 focus:outline-none font-medium rounded text-sm px-3 py-2.5 text-center cursor-pointer"
        }
      ...
      >
        {buyButtons.length > 0 ? buyButtons[0]?.label : "Comprar entradas"}
      </button>
    </a>
  );
};
